...
public class Globals {
	...
	static ArrayList<CartDraft> drafts = JsonLoader.loadDrafts();
	...
	public static void generateCart() {
		...
		for(CartDraft cd : drafts) {
			if(cd.getUserID()==((User)currentUser).getUserID()) {
				
				for(Integer i : cd.getProducts().keySet())
					cart.addProduct(barCodeTable.get(i), cd.getProducts().get(i));
	...
	public static void loadHistory() {
		...
		storico=JsonLoader.loadHistory();
